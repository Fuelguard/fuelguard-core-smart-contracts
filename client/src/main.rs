...
fn load_cfg(client_config: &String) -> Result<ClientConfig> {
    ...
    if solforex_v3_program_str.is_empty() {
        panic!("solforex_v3_program must not be empty");
    }
    let solforex_v3_program = Pubkey::from_str(&solforex_v3_program_str).unwrap();
    ...
    let (amm_config_key, __bump) = Pubkey::find_program_address(
        &[
            solforex_amm_v3::states::AMM_CONFIG_SEED.as_bytes(),
    ...
    let pool_id_account = if mint0 != None && mint1 != None {
        ...
        Some(
            Pubkey::find_program_address(
                &[
                    solforex_amm_v3::states::POOL_SEED.as_bytes(),
...
fn load_cur_and_next_five_tick_array(
    ...
    for tick_array in tick_array_rsps {
        let tick_array_state =
            deserialize_anchor_account::<solforex_amm_v3::states::TickArrayState>(
                &tick_array.unwrap(),
            )
            .unwrap();
...
fn get_nft_account_and_position_by_owner(
    client: &RpcClient,
    owner: &Pubkey,
    solforex_amm_v3_program: &Pubkey,
                    ...
                    serde_json::from_value(parsed_account.parsed)
                {
                    ...
                    if ui_token_account.token_amount.decimals == 0 && token_amount == 1 {
                        let (position_pda, _) = Pubkey::find_program_address(
                            &[
                                solforex_amm_v3::states::POSITION_SEED.as_bytes(),
                                token.to_bytes().as_ref(),
                            ],
                            &solforex_amm_v3_program,
                        );
...
fn main() -> Result<()> {
    ...
    let program = anchor_client.program(pool_config.solforex_v3_program)?;
    ...
    match opts.command {
        ...
        CommandsName::CreatePool {
            ...
            let mut create_observation_instr = create_account_rent_exmpt_instr(
                ...
                pool_config.solforex_v3_program,
                solforex_amm_v3::states::ObservationState::LEN,
            )?;
        ...
        CommandsName::InitReward {
            ...
            let reward_token_vault = Pubkey::find_program_address(
                &[
                    solforex_amm_v3::states::POOL_REWARD_VAULT_SEED.as_bytes(),
                    pool_config.pool_id_account.unwrap().to_bytes().as_ref(),
        ...
        CommandsName::OpenPosition {
            ...
            let pool: solforex_amm_v3::states::PoolState =
                program.account(pool_config.pool_id_account.unwrap())?;
            ...
            let tick_array_lower_start_index =
                solforex_amm_v3::states::TickArrayState::get_array_start_index(
                    tick_lower_index,
                    pool.tick_spacing.into(),
                );
                ...
                match rsp {
                    ...
                    Some(rsp) => {
                        let position = deserialize_anchor_account::<
                            solforex_amm_v3::states::PersonalPositionState,
                        >(&rsp)?;
                        user_positions.push(position);
                    }
                }
            }
            let mut find_position = solforex_amm_v3::states::PersonalPositionState::default();
        ...
        CommandsName::Swap {
            ...
            let user_output_state =
                spl_token::state::Account::unpack(&user_output_account.as_ref().unwrap().data)
                    .unwrap();
            let amm_config_state = deserialize_anchor_account::<solforex_amm_v3::states::AmmConfig>(
                amm_config_account.as_ref().unwrap(),
            )?;
            let pool_state = deserialize_anchor_account::<solforex_amm_v3::states::PoolState>(
                pool_account.as_ref().unwrap(),
            )?;
            let tickarray_bitmap_extension =
                deserialize_anchor_account::<solforex_amm_v3::states::TickArrayBitmapExtension>(
        ...
        CommandsName::PTickState { tick, pool_id } => {
            ...
            let pool: solforex_amm_v3::states::PoolState = program.account(pool_id)?;
            ...
            let mut tick_array_account: solforex_amm_v3::states::TickArrayState =
        ...
        CommandsName::POperation => {
            ...
            let operation_account: solforex_amm_v3::states::OperationState =
        ...
        CommandsName::PObservation => {
            ...
            let observation_account: solforex_amm_v3::states::ObservationState =
        ...
        CommandsName::PConfig { config_index } => {
            ...
            let amm_config_account: solforex_amm_v3::states::AmmConfig =
        ...
        CommandsName::TickArraryStartIndex { tick, tick_spacing } => {
            println!(
                ...
                tick,
                tick_spacing,
                solforex_amm_v3::states::TickArrayState::get_array_start_index(tick, tick_spacing,)
            );
        }
        CommandsName::LiquidityToAmounts {
            ...
            let pool_account: solforex_amm_v3::states::PoolState =
                program.account(pool_config.pool_id_account.unwrap())?;
            let amounts = solforex_amm_v3::libraries::get_delta_amounts_signed(
                ...
                RpcProgramAccountsConfig {
                    filters: Some(vec![
                        ...
                        RpcFilterType::DataSize(
                            solforex_amm_v3::states::PersonalPositionState::LEN as u64,
                        ),
                    ]),
        ...
        CommandsName::PProtocolPositionByPool { pool_id } => {
            ...
            let position_accounts_by_pool = rpc_client.get_program_accounts_with_config(
                ...
                RpcProgramAccountsConfig {
                    filters: Some(vec![
                        ...
                        RpcFilterType::DataSize(
                            solforex_amm_v3::states::ProtocolPositionState::LEN as u64,
                        ),
                    ]),
                    account_config: RpcAccountInfoConfig {
            ...
            for position in position_accounts_by_pool {
                let protocol_position = deserialize_anchor_account::<
                    solforex_amm_v3::states::ProtocolPositionState,
                >(&position.1)?;
        ...
        CommandsName::PTickArrayByPool { pool_id } => {
            ...
            let tick_arrays_by_pool = rpc_client.get_program_accounts_with_config(
                ...
                RpcProgramAccountsConfig {
                    filters: Some(vec![
                        ...
                        RpcFilterType::DataSize(solforex_amm_v3::states::TickArrayState::LEN as u64),
                    ]),
                    account_config: RpcAccountInfoConfig {
                        encoding: Some(UiAccountEncoding::Base64Zstd),
                        ..RpcAccountInfoConfig::default()
            ...
            for tick_array in tick_arrays_by_pool {
                let tick_array_state = deserialize_anchor_account::<
                    solforex_amm_v3::states::TickArrayState,
                >(&tick_array.1)?;
        ...
        CommandsName::PPool { pool_id } => {
            ...
            println!("pool_id:{}", pool_id);
            let pool_account: solforex_amm_v3::states::PoolState = program.account(pool_id)?;
        ...
        CommandsName::PBitmapExtension { bitmap_extension } => {
            ...
            let bitmap_extension_account: solforex_amm_v3::states::TickArrayBitmapExtension =
                program.account(bitmap_extension)?;
            println!("{:#?}", bitmap_extension_account);
        }
        CommandsName::PProtocol { protocol_id } => {
            let protocol_account: solforex_amm_v3::states::ProtocolPositionState =
                program.account(protocol_id)?;
            println!("{:#?}", protocol_account);
        }
        CommandsName::PPersonal { personal_id } => {
            let personal_account: solforex_amm_v3::states::PersonalPositionState =
        ...
        CommandsName::DecodeEvent { log_event } => {
            handle_program_log(
                &pool_config.solforex_v3_program.to_string(),
                &log_event,
                false,
            )?;
        }
        CommandsName::DecodeTxLog { tx_id } => {
            ...
            // decode logs
            parse_program_event(&pool_config.solforex_v3_program.to_string(), meta.clone())?;
        }
    }
